...
import { submitFinancingForm } from "@/lib/actions/leads";
import type { FinancingFormState } from "@/lib/definitions";
...
export default function FinancingForm({
  ...
  const [state, action, pending] = useActionState<FinancingFormState, FormData>(
    submitFinancingForm,
    undefined
  );
  ...
  return (
    <form action={action} className="flex flex-col gap-4">
      ...
      <p className="text-xs text-slate-500 dark:text-slate-400">{t("disclaimer")}</p>
    </form>
  );
}
